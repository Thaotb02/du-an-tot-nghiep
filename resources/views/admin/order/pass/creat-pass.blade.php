                  ...
                  <div class="col-sm-6">
                     <div class="card">
                        <div class="card-header">
                           <h3 style="font-weight: 600">Chuyển nhượng hóa đơn cho hội viên mới</h3>
                        </div>
                        <div class="card-block">
                           <form id="main" method="post"action="{{route('order.saveNewMemberPass',['id'=>$orders->id])}}" enctype="multipart/form-data">
                              @csrf
                              ...
                              <div class="form-group row mt-4">
                                 <div class="col-sm-6" >
                                    <label for=""> Người chuyển nhượng : {{$orders->member->infor->name}}</label>
                                 </div>
                                 <div class="col-sm-6" >
                                    <label for=""> Ngày bắt đầu gói tập  : {{$orders->start_date->format('d-m-Y')}}</label>
                                 </div>
                                 <div class="col-sm-6">
                                    <label >Huấn luyện viên:  @if($typePackage->catetoryPackage ==2)
                                       {{$orders->pt->infor->name}}
                                       @else 
                                       Không có 
                                       @endif</label>
                                 ...
                                 <div class="col-sm-6">
                                    <label for="">Loại gói tập:{{$typePackage->TypePackage_name}}</label>
                                    <input name="typePackage_id" type="hidden" value="{{$typePackage->id}}">
                                 </div>
                                 <div class="col-sm-6" >
                             
                                    <label for=""> Ngày kết thúc gói tập  : {{$orders->finish_date->format('d-m-Y')}}</label>
                                 </div>
                                 <div class="col-sm-6" >
                                   
                                    <label for=""> Bộ môn : {{$typePackage->subject->subject_name}}</label>
                                 </div>
                              ...
                              <div class="form-group row">
                               ...
                               @enderror
                                 <div class="col-sm-6">
                                    <label >Ngày kết thúc gói tập <span style="color: red">*</span></label>
                                    <input type="text" name="finish_date"
                                 class="form-control" value="{{$orders->finish_date->format('Y-m-d')}}">
                                 </div>
                              </div>
                              
                              <div class="form-group row mt-9">
                                 <div class="col-sm-7" >
                                    <label style="font-size: 24px; " >Phí chuyển nhượng: 1.000.000 VNĐ </label>
                                  
                                       <input type="hidden" name="total_money" value="1000000">
